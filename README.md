...
A high-performance, completely offline spellchecker for Obsidian using standard Hunspell `.aff` and `.dic` dictionary pairs. This plugin replaces the default browser spellchecker, providing advanced control over multiple languages, cross-platform stability (including Mobile support), and real-time word suggestions—without ever sending your private notes to third-party servers.

## ✨ Features

-   **High Performance on Mobile:** Specifically architected to avoid memory bloat. The spellchecking engine utilizes "on-the-fly" reverse affix evaluation, making it light enough to run smoothly on iOS and Android devices without causing Out-of-Memory (OOM) crashes.
...
-   **Native Look and Feel:** The red squiggly underlines mimic native system spellcheckers perfectly, adapting to your Obsidian theme (light or dark mode).

## 📦 Included by Default

For immediate usage, **Portuguese (Brazil) (`pt-BR`)** is included by default. If you only need this language, you just need to install and enable the plugin!
...
You can download hundreds of community-maintained Hunspell dictionaries directly from the official LibreOffice repository:
